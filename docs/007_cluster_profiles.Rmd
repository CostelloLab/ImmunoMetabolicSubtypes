...
kmeans_clusters = kmeans(differential_expression$FC, centers = 4)
...
pdf("~/OneDrive - The University of Colorado Denver/Projects/ImmunoMetabolicSubtypes/results/cluster_heatmaps/T21_cyt_met_diff_clinic_4clusters_25neighbors_heatmap.pdf", width = 6, height = 4)
...
full_clinic <- rbind(T21.mixed$clinic,D21.mixed$clinic)
D21 <- ifelse(D21.mixed$clinic$Karyotype == "Control", 5,"")
names(D21) <- D21.mixed$clinic$LabID
clustering_D21 <- c(all_clust$clustering, D21)

karyotype_associations <- association_test(clinic = full_clinic,
                                         clustering = clustering_D21 ,
                                         phenotypes = names(T21.mixed$clinic)[10:21])
