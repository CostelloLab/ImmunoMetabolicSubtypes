...
p1 = ggplot(omics.clinic, aes_string(y = sprintf("`%s`", feat), x = 'clustering', fill = 'clustering' ) ) +
	geom_boxplot(alpha = .7) +
	geom_beeswarm() +
    theme_classic() +
    	scale_fill_manual(values = c("1" = "lightgoldenrod","2" = "salmon", "3" = "darkorchid", "4" = "green4","5" = "royalblue" )) +
	theme(axis.title = element_text(size = 24), 
		...
		ggtitle(feat)+
		xlab("Integrated Omics \n Cluster")+
		ylab("Standardized Abundance")  +
		guides(alpha = "none")
	pdf(file = sprintf("./results/figures/Cluster_sina_plots/%s.pdf", gsub("/", " ", feat), width = 3))
	print(p1)
	dev.off()
}
```
