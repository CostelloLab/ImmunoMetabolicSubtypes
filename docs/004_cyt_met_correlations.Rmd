...
T21_correlations$long$cohort <- "all T21"
T21_sig_correlations <- T21_correlations$long %>%
    filter(fdr < .1)
T21_sig_correlations <- T21_sig_correlations %>%
    mutate(cyt_met = paste0(A,"-", B))


for(x in 1:10){
    sub_corr[[x]]$cohort <- paste0("random sample ",x)
    sub_corr[[x]] <- sub_corr[[x]] %>%
        mutate(cyt_met = paste0(A,"-", B)) %>%
        filter(cyt_met %in% T21_sig_correlations$cyt_met)
}

all_correlations <- bind_rows(sub_corr,T21_sig_correlations,)

p1 <- ggplot(all_correlations, aes(x = r, fill = cohort)) +
    geom_density(alpha = .3) +
    theme_classic() +
    ylab("Count") +
    xlab("Correlation Coefficient")

pdf("~/OneDrive - The University of Colorado Denver/Projects/ImmunoMetabolicSubtypes/results/correlation_density_plots/sub_sampling_density_plots.pdf", width = 8, height = 6)
print(p1)
dev.off()
