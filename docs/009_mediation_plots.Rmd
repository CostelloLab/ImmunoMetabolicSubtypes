...
pathwayAnnotateZ <- function(relationship, long_z_tibble, pathway_genes) {
    output <- long_z_tibble %>%
        ...
        mutate(pathway = ifelse(gene %in% pathway_genes, "in pathway", "other"))
    return(output)
}
               
ifn_z <- pathwayAnnotateZ(IFN_cyt_mets$cyt_met[1], long_z[[1]],pathways_all$HALLMARK_INTERFERON_GAMMA_RESPONSE)
