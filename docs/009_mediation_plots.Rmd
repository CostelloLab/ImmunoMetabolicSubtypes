...
Combine Z scores over pathway within cluster
```r
cluster_z_scores <- lapply(names(pathways_all), function(x) clusterPathwayZscores(x,gsea_results))
names(cluster_z_scores) <- names(pathways_all)

cluster_z_scores <- lapply(cluster_z_scores, unlist)
cluster_z_scores <- lapply(cluster_z_scores, as.data.frame)
cluster_z_scores <- lapply(cluster_z_scores, setNames, c("T21", "D21", 1,2,3,4))
cluster_z_scores <- cluster_z_scores %>% bind_rows()
rownames(cluster_z_scores) <- names(pathways_all)
cluster_z_scores <- cluster_z_scores[-nrow(cluster_z_scores),]
rownames(cluster_z_scores) <- gsub("HALLMARK", "", rownames(cluster_z_scores))
rownames(cluster_z_scores) <- gsub("_", " ", rownames(cluster_z_scores))

col_fun <- colorRamp2(c(1.5,2), c("grey", "red"))
p1 <- Heatmap(as.matrix(cluster_z_scores),
              name = "NES",
              column_order= names(cluster_z_scores),
              show_row_dend = F,
              col = col_fun,
              row_names_side = "left",
