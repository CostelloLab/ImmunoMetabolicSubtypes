...
single_rel_z <- long_z[[1]] %>%
  ...
  arrange(-combined_Z) %>%
    ...
    mutate(pathway = gene %in% pathways_all$HALLMARK_INTERFERON_GAMMA_RESPONSE) %>%
    mutate(label = ifelse((combined_Z >5) | gene %in%  c("IDO1") , gene, ""))


p1 <- ggplot(single_rel_z, aes(x = rank, y = combined_Z)) +
    geom_point(size = .2)+
    geom_point(data = single_rel_z[single_rel_z$pathway == TRUE,], aes(x = rank, y = combined_Z),  color ="red", size = .2)+
    theme_classic() +
    xlab("rank") +
    ylab("Z score") +
    ggtitle(paste0(key_cyt_met))+
...
pdf(paste0("~/OneDrive - The University of Colorado Denver/Projects/ImmunoMetabolicSubtypes/results/mediation_trendplots/",key_cyt_met,"_Z_trend_plot_",Sys.Date(),".pdf"), width = 2, height = 2.5)
print(p1)
dev.off()
...
single_gene_z <- long_z[[1]] %>%
  ...
  arrange(-combined_Z) %>%
    mutate(rank = row_number())%>%
    mutate(label = ifelse(combined_Z > 5,cyt_met, "")) %>%
    mutate(ifn = grepl("IFN-gamma", cyt_met))

p1 <- ggplot(single_gene_z, aes(x = rank, y = combined_Z)) +
    geom_point(size = .2) +
    geom_point(data = single_gene_z[single_gene_z$ifn == TRUE,],aes(x = rank, y = combined_Z), color = "red",size = .2)+
    theme_classic() +
    xlab("rank") +
    ylab("Z score")+
    ylim(c(-6,6))+
...
pdf(paste0("~/OneDrive - The University of Colorado Denver/Projects/ImmunoMetabolicSubtypes/results/mediation_trendplots/",key_gene,"_Z_trend_plot_",Sys.Date(),".pdf"), width = 2.5, height = 2.5)
print(p1)
dev.off()
...
## plot mediation for top cyt-metabolic

tmp <- long_z[[1]] %>%
    filter(cyt_met == IFN_cyt_mets$cyt_met[1]) %>%
    mutate(pathway = ifelse(gene %in% pathways_all$HALLMARK_INTERFERON_GAMMA_RESPONSE, "IFN-gamma", "other")) %>%
    mutate(pathway = factor(pathway, levels = c("other", "IFN-gamma")))

library(ggbeeswarm)
p1 <- ggplot(tmp, aes(x = pathway, y = combined_Z, color = pathway))+
    geom_quasirandom(size = .1)+
    geom_boxplot(outlier.size = .1, alpha = .6) + 
    theme_classic() +
    ggtitle(IFN_cyt_mets$cyt_met[[1]]) +
    theme(axis.text = element_text(size = 6),
          plot.title = element_text(size = 6),
          axis.title = element_text(size = 6),
          legend.position = "none"
          )
pdf(paste0("~/OneDrive - The University of Colorado Denver/Projects/ImmunoMetabolicSubtypes/results/mediation_boxplots/IFN-gamma-",IFN_cyt_mets$cyt_met[1],"_", Sys.Date(),".pdf"), height = 1.5, width = 1.5)
print(p1)
dev.off()
...
tmp <- long_z[[1]] %>%
    filter(cyt_met == heme_heme_cyt_mets$cyt_met[1]) %>%
    mutate(pathway = ifelse(gene %in% pathways_all$HALLMARK_HEME_METABOLISM, "Heme\nMetabolism", "other")) %>%
    mutate(pathway = factor(pathway, levels = c("other", "Heme\nMetabolism")))
p1 <- ggplot(tmp, aes(x = pathway, y = combined_Z, color = pathway))+
    geom_quasirandom(size = .1)+
    geom_boxplot(outlier.size = .1, alpha = .6) + 
    theme_classic() +
    ggtitle(heme_heme_cyt_mets$cyt_met[[1]]) +
    theme(axis.text = element_text(size = 6),
          plot.title = element_text(size = 6),
          axis.title = element_text(size = 6),
          legend.position = "none"
          )
pdf(paste0("~/OneDrive - The University of Colorado Denver/Projects/ImmunoMetabolicSubtypes/results/mediation_boxplots/heme--",heme_heme_cyt_mets$cyt_met[1],"_", Sys.Date(),".pdf"), height = 1.5, width = 1.5)
print(p1)
dev.off()
...
tmp <- long_z[[1]] %>%
    filter(cyt_met == op_op_cyt_mets$cyt_met[1]) %>%
    mutate(pathway = ifelse(gene %in% pathways_all$HALLMARK_OXIDATIVE_PHOSPHORYLATION, "Oxidative\nPhosphorylation", "other")) %>%
    mutate(pathway = factor(pathway, levels = c("other", "Oxidative\nPhosphorylation")))
p1 <- ggplot(tmp, aes(x = pathway, y = combined_Z, color = pathway))+
    geom_quasirandom(size = .1)+
    geom_boxplot(outlier.size = .1, alpha = .6) + 
    theme_classic() +
    ggtitle(op_op_cyt_mets$cyt_met[[1]]) +
    theme(axis.text = element_text(size = 6),
          plot.title = element_text(size = 6),
          axis.title = element_text(size = 6),
          legend.position = "none"
          )
pdf(paste0("~/OneDrive - The University of Colorado Denver/Projects/ImmunoMetabolicSubtypes/results/mediation_boxplots/OxPhos--",op_op_cyt_mets$cyt_met[1],"_", Sys.Date(),".pdf"), height = 1.5, width = 1.5)
print(p1)
dev.off()
...
x <- list(IFN_cyt_mets$cyt_met, heme_heme_cyt_mets$cyt_met, op_op_cyt_mets$cyt_met)
names(x) <- gsub("HALLMARK_", "", DS_pathways)
names(x) <- c("IFN-  Response", "Heme Metabolism","Oxidative Phosphorylation")
...
ggsave(filename = "~/OneDrive - The University of Colorado Denver/Projects/ImmunoMetabolicSubtypes/results/venn_diagrams/mediation_signatures_venn.pdf", width = 2, height =2, units ="in")
