...
```{r density plots}
key_gene = "STAT1"
single_gene_z <-long_z[[1]]%>%
  filter(gene == key_gene) %>%
  as.data.frame() %>%
  arrange(-combined_Z)

tail(single_gene_z,20)

p1 <- ggplot(single_gene_z, aes(x = combined_Z)) +
  geom_density()+
  geom_vline(xintercept = single_gene_z[single_gene_z$`cyt_met` == "IFN-gamma-kynurenine","combined_Z"], color ="red")+
    annotate("text", x=  single_gene_z[single_gene_z$`cyt_met` == "IFN-gamma-kynurenine","combined_Z"]-.3,y = .15, angle = 90,  label="IFN-gamma-kynurenine", color = "red", size = 6)+
  # geom_vline(xintercept = single_gene_z[single_gene_z$`cyt_met` == "TNF-alpha-kynurenine", "combined_Z"], color = "blue") +
  #   annotate("text", x=  single_gene_z[single_gene_z$`cyt_met` == "TNF-alpha-kynurenine", "combined_Z"]-.3,y = .15, angle = 90,  label="TNF-alpha-kynurenine", color = "blue", size = 6)+
  geom_vline(xintercept = single_gene_z[single_gene_z$`cyt_met`=="IL-1RA-Glutamate", "combined_Z"], color = "purple")+
    annotate("text", x= single_gene_z[single_gene_z$`cyt_met`=="IL-1RA-Glutamate", "combined_Z"]-.3,y = .15, angle = 90,  label="IL-1RA-Glutamate", color = "purple", size = 6)+
  ggtitle("STAT1") + 
  theme_classic() +
  xlab("combined_Z") +
  ylab("density")
...
pdf(paste0("~/OneDrive - The University of Colorado Denver/Projects/subPhenoDS/results/partial_correlation_results/figures/",key_gene,"density_plot.pdf"), width = 10, height = 6)
print(p1)
dev.off()
...
length(single_rel_z$gene) -which(single_rel_z$gene == "IDO1")
length(single_rel_z$gene)

p1 <- ggplot(single_rel_z, aes(x = combined_Z)) +
  geom_density()+
  geom_vline(xintercept = single_rel_z[single_rel_z$gene == "IDO1","combined_Z"], color ="red")+
  annotate("text", x= single_rel_z[single_rel_z$gene == "IDO1","combined_Z"]-.3,y = .5, angle = 90,  label="IDO1", color = "red", size = 6)+
  geom_vline(xintercept = single_rel_z[single_rel_z$gene == "STAT1","combined_Z"], color ="darkgreen")+
    ggtitle("IFN-gamma-kynurenine") + 
   annotate("text", x= single_rel_z[single_rel_z$gene == "STAT1","combined_Z"]-.3,y = .5, angle = 90,  label="STAT1", color = "darkgreen", size = 6)+
  theme_classic() +
  xlab("combined_Z") +
  ylab("density")

pdf(paste0("~/OneDrive - The University of Colorado Denver/Projects/subPhenoDS/results/partial_correlation_results/figures/",key_cyt_met,"density_plot.pdf"), width = 10, height = 6)
print(p1)
dev.off()
...
p1 <- ggplot(single_rel_z, aes(x = rank, y = combined_Z)) +
  ...
  annotate("text", y= single_rel_z[single_rel_z$cyt_met == "IFN-gamma-kynurenine","combined_Z"],x =single_rel_z[single_rel_z$cyt_met == "IFN-gamma-kynurenine","rank"]+ 200, angle = 0,  label="IFN-gamma-kynurenine", color = "red", size = 6) +
  geom_point(data = single_rel_z[single_rel_z$cyt_met == "IL-1RA-Glutamate",], aes(x = rank, y = combined_Z),  color ="green")+
  annotate("text", y= single_rel_z[single_rel_z$cyt_met == "IL-1RA-Glutamate","combined_Z"],x =single_rel_z[single_rel_z$cyt_met == "IL-1RA-Glutamate","rank"]- 150, angle = 0,  label="IL-1RA-Glutamate", color = "green", size = 6)+
  theme_classic() +
  xlab("rank") +
  ylab("Z score")
...
## Heatmaps of gsea results
  ...
  * Creating heatmaps from the gsea results. Will make an overall heatmap to begin with and then stratify based on the directionality of the relationships. 

```{r global heatmap data}
...
# toplot_clusters_pos <- lapply(1:7, function(x) {
#    toplot_clusters[[x]] %>% 
#     dplyr::select(which(names(toplot_clusters[[x]]) %in% cyt_met_pos))
# })
# 
# 
# toplot_clusters_neg <- lapply(1:7, function(x) {
#    toplot_clusters[[x]] %>% 
#     dplyr::select(which(names(toplot_clusters[[x]]) %in% cyt_met_neg))
# 
# })

```

```{r mediated by genes in both pathways}
...
multiMediators(toplot_clusters[[1]], DS_pathways[c(1,3)])
```
...
cyt_path <- read.csv("/Users/lucas/OneDrive - The University of Colorado Denver/Projects/subPhenoDS/data/cytokine_annotation.csv")



 
aggRank <- function(key_pathway) {
  
  
  
  cyt_met_examples <- gsea_results_formatted[[1]] %>%                                                  
    rownames_to_column("pathway") %>%
...
ifg <- aggRank( "HALLMARK_INTERFERON_GAMMA_RESPONSE")
heme <- aggRank("{HALLMARK_HEME_METABOLISM")
