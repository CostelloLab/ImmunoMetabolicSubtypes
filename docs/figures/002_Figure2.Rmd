...
library(tidyverse)
library(ggplot2)
load("~/OneDrive - The University of Colorado Denver/Projects/subPhenoDS/results/partial_correlation_results/long_z_11_27.RData")
```
...
p1 <- ggplot(single_rel_z, aes(x = rank, y = combined_Z)) +
  ...
  geom_point(data = single_rel_z[single_rel_z$cyt_met == "IFN-gamma-kynurenine",], aes(x = rank, y = combined_Z),  color ="red")+
    ...
    geom_point(data = single_rel_z[single_rel_z$cyt_met == "MIP-1beta-Icosatetraenoic acid",], aes(x = rank, y = combined_Z),  color ="darkgreen")+
  annotate("text", y= single_rel_z[single_rel_z$cyt_met == "MIP-1beta-Icosatetraenoic acid","combined_Z"],x =single_rel_z[single_rel_z$cyt_met == "MIP-1beta-Icosatetraenoic acid","rank"]+ 300, angle = 0,  label="MIP-1beta-Icosatetraenoic acid", color = "darkgreen", size = 6)+
  theme_classic() +
  xlab("rank") +
  ylab("Z score")
...
```{r  data}
library(tidyverse)
load("~/OneDrive - The University of Colorado Denver/Projects/subPhenoDS/results/partial_correlation_results/partial_corrlation_gsea_results_formatted_11_27.RData")
...
multiHeatmapOrderedGSEA(toplot1 = toplot1,
                        ...
                        formatted_gsea = gsea_results_formatted[[1]] ,
                        full_results = T21_filtered_full_results,
                        output_file = "~/OneDrive - The University of Colorado Denver/Projects/subPhenoDS/results/partial_correlation_results/figures/2D_filtered_ordered-E2F-TARGETS.pdf",
                        order_pathway = "HALLMARK_E2F_TARGETS",
                        col_vector = col_vector)
...
cyt_mets <- filterCyt_Mets(toplot = toplot1,
               formatted_gsea = gsea_results_formatted[[1]],
               key_pathway = "HALLMARK_E2F_TARGETS",
               gene_sets = pathways_all,
               num_cyt_met = 15)
cluster_ranks <- clusterGeneRanks(cyt_mets = cyt_mets,
                                  long_z_list = long_z,
                                  ...
                                  gene_sets = pathways_all)
names(cluster_ranks) <- names(long_z)
cluster_heatmaps <- clusterHeatmapWrapper(cluster_ranks, name = "E2F Targets", output_file = "~/OneDrive - The University of Colorado Denver/Projects/subPhenoDS/results/partial_correlation_results/figures/cluster_heatmaps_E2F-Targets-ranks.pdf")
