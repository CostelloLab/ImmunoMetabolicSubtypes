...
## NEMO way to find the number of  neighbors for clustering
The NEMO clustering approach requires one to choose the number of neighbors to include in the creation of the similarity matrix. In the paper they sugest that one do so based on this formula: #samples/#clusters if one knows the number of clusters. To evaluate this, we explored the clustering solutions using this approach. 
...
res <- clusterEval(omics.list = omics.list,
...
An alternative approach may consider that we are interested in finding axes of disease co-occurance among the data. Therefore, we will restrict our number of neighbors to the mean number of neighbors. 
